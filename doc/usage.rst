...
public and private keys, manage trust and query a keys expiry::
   ...
   # This backend requires a gnupg_version for certain operations, we assume this comes from an
   # environment variable GNUPG_VERSION.
   >>> import os
   >>> gpgver = tuple([int(t) for t in os.environ.get('GNUPG_VERSION', '2').split('.')])
   
   >>> backend = gpgme.GpgMeBackend(home=gnupg_home, gnupg_version=ggpver)
...
Signing and/or encrypting is straight forward::
   ...
   >>> from gpgliblib import gpgme
   >>> backend = gpgme.GpgMeBackend(home=gnupg_home, default_trust=True, gnupg_version=gpgver)
   >>> fingerprint = 'CC9F343794DBB20E13DE097EE53338B91AA9A0AC'
